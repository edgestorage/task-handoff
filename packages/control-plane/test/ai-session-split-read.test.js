...
test("v0.0.23 complete detail is projected into split detail, index, and Turn body", async () => {
  ...
  assert.deepEqual((await service.turnIndex("instance-1", legacy.id)).turns.map((turn) => turn.id), ["turn-1"]);
  assert.equal((await service.turnBody("instance-1", legacy.id, "turn-1")).turn.lastMessage, "answer");
