...
import type { NodeAgentTransport } from "./node-agent-client.ts";
import type { ControlPlaneEventBus } from "./events.ts";
import { createNodeAgentHmacHeaders, hmacHeadersFromRecord, sha256Hex, signNodeAgentRequest, timingSafeHexEqual, NODE_AGENT_HMAC_TIMESTAMP_WINDOW_MS } from "./node-agent-auth.ts";
import { createNodeAgentIpcWebSocket, parseNodeAgentIpcEndpoint } from "./node-agent-ipc.ts";
import { EventConnectionRetryTimer, eventConnectionSafetyIntervalMs } from "./event-connection-retry.ts";
...
export class ControlPlaneNodeAgentTunnelTransport implements NodeAgentTransport {
  ...
  attach(nodeId: string, socket: { send: (data: string) => void; readyState?: number; on?: (event: string, listener: (...args: unknown[]) => void) => void }) {
    ...
    this.sockets.set(nodeId, { socket, pending, streams: new Map() });
