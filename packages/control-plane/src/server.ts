...
import { CONTROL_PLANE_SESSION_COOKIE, ControlPlaneAuth, type ControlPlaneAuthOptions } from "./auth.ts";
import { ControlPlaneService, type ControlPlaneServiceOptions } from "./service.ts";
...
import { ControlPlaneNodeAgentTunnelTransport, ControlPlaneNodeEventSubscriber } from "./node-agent-tunnel.ts";
import { controlPlaneStorePaths } from "./store.ts";
import { acquireProcessSingletonLock, defaultControlPlaneSingletonLockPath } from "./process-lock.ts";
import { assertCan, type ControlPlaneAction, type ControlPlaneActor, type ControlPlaneResource } from "./authorization.ts";
import { registerControlPlaneManagementRoutes } from "./server-management-routes.ts";
import { registerInstanceProxyRoutes } from "./server-instance-proxy-routes.ts";
import { ControlPlaneAiSessionAggregator } from "./ai-session-aggregator.ts";
import { ControlPlaneAppSessionAggregator } from "./app-session-aggregator.ts";
import { nodeAgentInstallScript } from "./install-scripts.ts";
...
export async function runControlPlaneServer(options: RunControlPlaneServerOptions) {
  const paths = controlPlaneStorePaths(options.dataDir);
  const lock = acquireProcessSingletonLock(defaultControlPlaneSingletonLockPath(), {
    component: "control-plane",
    dataDir: paths.dataDir,
    host: options.host,
    port: options.port,
