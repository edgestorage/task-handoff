...
import { ControlPlaneNodeAgentClient, type NodeAgentTransport, type NodeAgentWebSocket } from "./node-agent-client.ts";
import { ControlPlaneNodeAgentGateway } from "./node-agent-gateway.ts";
import { ControlledInstanceGateway } from "./controlled-instance-gateway.ts";
import { InstanceBoardReader } from "./instance-board-reader.ts";
import { ControlPlaneTriggerService } from "./control-plane-trigger-service.ts";
import { AppAccessService, type AppAccessMode } from "./app-access-service.ts";
import { ChatActionTokenService, type ChatActionToken } from "./chat-action-token-service.ts";
import { ChatBridgeService } from "./chat-bridge-service.ts";
...
} from "./service-inputs.ts";
import { createId, createSecret, JsonCollection, JsonFile, type ControlPlaneStorePaths, type StoredRecord } from "./store.ts";
import { controlledInstanceTriggerSnapshot } from "./trigger-records.ts";
import { createNodeAgentHmacHeaders } from "./node-agent-auth.ts";
import { assertLocalIpcSocketOwnedByCurrentUser, createNodeAgentIpcWebSocket, fetchNodeAgentIpc, parseNodeAgentIpcEndpoint } from "./node-agent-ipc.ts";
...
export class ControlPlaneService {
  ...
  readonly images: JsonCollection<ImageProfile>;
  ...
  readonly settings: JsonFile<ControlPlaneSettings>;
  ...
  constructor(paths: ControlPlaneStorePaths, options: ControlPlaneServiceOptions = {}) {
    ...
    this.chatSessionRuntime = new ControlPlaneChatSessionRuntime({
      ...
      getProject: (id) => this.projects.get(id),
  ...
  getSettings() {
    return ControlPlaneSettingsSchema.parse(this.settings.get());
  }
  ...
  updateSettings(input: unknown) {
    ...
    return this.settings.put(next);
  }

  seedDefaults() {
    if (!this.images.get("img_default")) {
      const timestamp = now();
      this.images.put(
  ...
  listProjects() {
    return this.projects.list().map((project) => this.normalizeProjectRecord(project));
  }
  ...
  updateProject(id: string, input: unknown) {
    ...
    return this.projects.put(updated);
  }
  ...
  createImage(input: unknown) {
    ...
    const timestamp = now();
    const image = ImageProfileSchema.parse({
      ...parsedInput,
  ...
  updateImage(id: string, input: unknown) {
    ...
    const updated = ImageProfileSchema.parse({
      ...current,
      ...parsedInput,
      id,
      createdAt: current.createdAt,
    ...
    return this.images.put(updated);
  }
  ...
  async listPendingRoutes() {
    ...
    for (const instance of await this.listNodeInstances()) {
      ...
      const project = instance.projectId ? this.projects.get(instance.projectId) : undefined;
  ...
  requireProject(id: string) {
    ...
    if (!record) {
      throwNotFound("PROJECT_NOT_FOUND", `Project ${id} was not found.`);
    }
    return this.normalizeProjectRecord(record);
  }
  ...
  requireImage(id: string) {
    ...
    if (!record) {
      throwNotFound("IMAGE_NOT_FOUND", `Image ${id} was not found.`);
    }
    return record;
  }
