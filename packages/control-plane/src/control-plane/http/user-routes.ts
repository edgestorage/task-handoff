...
export function registerControlPlaneUserRoutes(app: FastifyInstance, auth: ControlPlaneAuth) {
  ...
  app.get("/api/users/:id", async (request) => ({ data: await auth.users.detail(IdParamsSchema.parse(request.params).id) }));
  ...
  app.get("/api/users/:id/sessions", async (request) => {
    ...
    return { data: await auth.sessions.listSessions(actor.userId, IdParamsSchema.parse(request.params).id) };
  });
  ...
  app.delete("/api/users/:id/identities/:identityId", async (request) => {
    ...
    const result = await auth.users.unbindExternalIdentity(params.id, params.identityId);
    await auth.notifyAuthorizationChanged(params.id);
    return { data: result };
  });
