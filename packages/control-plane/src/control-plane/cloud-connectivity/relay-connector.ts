...
export type RelaySessionBridge = {
  ...
  subscribe(actor: ControlPlaneActor, topics: string[], listener: (event: unknown) => void, aiSessionTransient?: AiSessionTransientSubscription): () => void;
...
class EncryptedControlPlaneRelaySession {
  ...
  private async dispatch(value: RelayEnvelope) {
    ...
    if (value.type === "event-subscribe") {
      ...
      this.unsubscribe = this.bridge.subscribe(this.actor, topics, (event) => this.send({ type: "event", event }), transient.success ? transient.data : undefined);
      return;
    }
