...
import type { ControlPlaneAiSessionSnapshotUpdate } from "./ai-session-aggregator.ts";
import { answerTelegramCallback, deleteTelegramMessage, editTelegramMessage, sendTelegramMessage, type TelegramMessageOptions } from "./telegram-gateway.ts";
import {
  dingdingCardUpdateResponse,
  createChatGatewaySendAdapter,
  DingdingProgressStore,
...
export function createDingdingStreamClient(input: { clientId: string; clientSecret: string }) {
  return new DWClient({
    ...
    // dingtalk-stream 2.1.5 sends heartbeat pings from an uncaught timer and can
    // throw while the socket is still CONNECTING. Keep SDK reconnects, but avoid
    // the unsafe heartbeat so a bridge failure cannot crash the control plane.
    keepAlive: false,
  });
}
...
export class ControlPlaneChatGatewayRuntime {
  ...
  private readonly createDingdingClient: (input: { clientId: string; clientSecret: string }) => DingdingClientLike;
  private readonly logger: ChatGatewayLogger | undefined;
  private bridgeTimers = new Map<string, Timer>();
  private dingdingBridges = new Map<string, DingdingRuntimeBridge>();
  ...
  private dingdingProgress: DingdingProgressStore;
  ...
  constructor(service: ChatGatewayService, fetchImpl: typeof fetch = fetch, options: ControlPlaneChatGatewayRuntimeOptions = {}) {
    ...
    this.dingdingProgress = new DingdingProgressStore(this.fetchImpl);
    ...
    this.createDingdingClient = options.createDingdingClient || createDingdingStreamClient;
  }
  ...
  stopBridge(id: string) {
    ...
    const dingding = this.dingdingBridges.get(id);
    if (dingding) {
      try {
        dingding.client.disconnect();
      } catch {
        // Ignore SDK disconnect races.
      }
      this.dingdingBridges.delete(id);
    }
    this.dingdingProgress.clearBridge(id);
    return this.status();
  }

  stopAll() {
    for (const id of new Set([...this.bridgeTimers.keys(), ...this.dingdingBridges.keys()])) {
      this.stopBridge(id);
    }
  ...
  status(): ChatGatewayRuntimeStatus {
    const bridges = this.service.listChatBridges().map((bridge) => ({
      ...
      channel: bridge.channel,
      name: bridge.name,
      running: this.bridgeTimers.has(bridge.id) || this.dingdingBridges.has(bridge.id),
  ...
  private startDingdingBridge(bridge: ChatBridgeConfig) {
    ...
    const clientSecret = stringSetting(settings.clientSecret);
    if (!bridge.token || !clientSecret) {
      this.bridgeErrors.set(bridge.id, "DingDing client id/secret is not configured.");
      return this.status();
    }
    const runtime: DingdingRuntimeBridge = {
      client: this.createDingdingClient({ clientId: bridge.token, clientSecret }),
      chatWebhooks: new Map(),
      senderIds: new Map(),
      conversationTypes: new Map(),
      onLog: (level, data, message) => {
        if (level === "warn") {
          this.logWarn({ bridgeId: bridge.id, ...data }, message);
          return;
        }
        this.logInfo({ bridgeId: bridge.id, ...data }, message);
      },
    };
    runtime.client.registerCallbackListener(TOPIC_ROBOT, (message) => {
      void this.handleDingdingRobotMessage(bridge, runtime, message as DWClientDownStream).catch((error) => {
    ...
    runtime.client.registerCallbackListener(TOPIC_CARD, (message) => {
      void this.handleDingdingCardCallback(bridge, runtime, message as DWClientDownStream)
        .then((result) => runtime.client.socketCallBackResponse((message as DWClientDownStream).headers.messageId, result || {}))
        .catch((error) => {
          this.bridgeErrors.set(bridge.id, error instanceof Error ? error.message : String(error));
          runtime.client.socketCallBackResponse((message as DWClientDownStream).headers.messageId, {});
        });
    });
    this.bridgeErrors.delete(bridge.id);
    this.dingdingBridges.set(bridge.id, runtime);
    runtime.client.connect()
      .then(() => {
        if (this.dingdingBridges.get(bridge.id) === runtime) {
          this.bridgeErrors.delete(bridge.id);
        }
      })
      .catch((error: unknown) => {
        if (this.dingdingBridges.get(bridge.id) === runtime) {
          this.dingdingBridges.delete(bridge.id);
          this.bridgeErrors.set(bridge.id, error instanceof Error ? error.message : String(error));
        }
      });
    this.startPendingPolling();
  ...
  private async sendDingdingRoutedAiSessionProgress(
    ...
    const delivered = await this.dingdingProgress.applyUpdate({
      bridge,
      dingdingRuntime: runtime,
      key,
      chatId,
      text,
  ...
  private async updateChatAiSessionProgress(binding: { bridgeId?: string; chatSessionId: string }, text: string, key: string, actionRows?: Array<Array<{ text: string; callbackData: string }>>) {
    ...
    if (bridge.channel === "dingding") {
      ...
      const updated = await this.dingdingProgress.applyUpdate({
        bridge,
        dingdingRuntime: runtime,
        key,
        chatId: binding.chatSessionId,
