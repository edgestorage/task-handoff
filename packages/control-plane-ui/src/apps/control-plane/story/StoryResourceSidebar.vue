  ...
  <aside class="story-resource-sidebar" tabindex="-1" :aria-label="t('stories.resources.title')">
    <header class="story-resource-header">
      <StoryResourceTabStrip
        class="story-resource-tab-strip"
        :items="tabItems"
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent class="app-launch-menu" align="end" :collision-padding="12" :side-offset="8">
          <DropdownMenuLabel class="story-resource-target-label">
            ...
            <span class="story-resource-menu-copy">
              <strong>{{ targetInstance?.name }}</strong>
              <small>{{ targetInstance?.node?.name || targetInstance?.nodeId }}</small>
            </span>
          </DropdownMenuLabel>
...
import { CircleAlert, FileDiff, FolderTree, GitBranch, PanelRight, Plus } from "@lucide/vue";
...
import { buildAppSessionTabs, EMBEDDED_BROWSER_APP_ID, launchableAppsForInstance, type RepositoryWorkspaceTabTarget, type SessionTab } from "../useInstanceSessions";
...
const emit = defineEmits<{
  ...
  openRepository: [instanceId: string, sessionKind: RepositorySessionKind, sessionId: string, page: StoryRepositoryPage, filePath?: string];
}>();
const { t } = useI18n();
...
function openTargetRepository(page: StoryRepositoryPage) {
  ...
  emit("openRepository", props.targetInstance.id, "ai-session", props.targetAiSessionId, page);
}
...
function openRepositoryFromContent(target: RepositoryWorkspaceTabTarget) {
  const resource = activeResource.value;
  if (!resource) return;
  emit("openRepository", resource.instanceId, target.sessionKind, target.sessionId, target.page === "changes-review" || target.page === "worktrees" ? target.page : "files", target.filePath);
}
...
.story-resource-tab-strip { min-width:0; flex:1; border-bottom:0; }
...
:global(.story-resource-target-label) { display:flex; align-items:center; gap:8px; min-width:0; }
:global(.story-resource-menu-copy) { display:grid; min-width:0; flex:1; gap:2px; }
:global(.story-resource-menu-copy strong), :global(.story-resource-menu-copy small) { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; letter-spacing:0; }
:global(.story-resource-menu-copy strong) { font-size:12px; font-weight:500; }
:global(.story-resource-menu-copy small) { color:var(--text-muted); font-size:12px; font-weight:400; }
