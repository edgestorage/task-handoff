<template>
  ...
  <AlertDialog :open="Boolean(pendingDelete)" @update:open="(open) => !open && (pendingDelete = undefined)">
    <AlertDialogContent>
      ...
      <AlertDialogFooter><AlertDialogCancel>{{ t("common.actions.cancel") }}</AlertDialogCancel><AlertDialogAction :disabled="Boolean(deletingModelId)" @click="confirmDelete">{{ deletingModelId ? t("settings.modelRegistry.deleting") : t("common.actions.delete") }}</AlertDialogAction></AlertDialogFooter>
    </AlertDialogContent>
  </AlertDialog>
