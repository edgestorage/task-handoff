...
test("session preview splits the original tab row into pane-aligned tab groups", async () => {
  ...
  assert.match(styles, /\.session-preview-primary-tools\.split \{[\s\S]*position:\s*relative;[\s\S]*grid-template-columns: minmax\(0, var\(--session-left-ratio\)\) minmax\(0, calc\(100% - var\(--session-left-ratio\)\)\)/);
  ...
  assert.match(styles, /\.session-pane-layout\.split\s*\{[\s\S]*grid-template-columns: minmax\(0, var\(--session-left-ratio\)\) minmax\(0, calc\(100% - var\(--session-left-ratio\)\)\)/);
  assert.match(styles, /\.session-pane-layout\.split \.session-pane\s*\{[\s\S]*grid-row:\s*1/);
  assert.match(styles, /\.session-pane-layout\.split \.session-pane\[data-pane="right"\]\s*\{[\s\S]*border-left:\s*2px solid var\(--line\)/);
  assert.match(styles, /\.session-pane-layout\.split \.session-pane-resize-handle::after\s*\{[\s\S]*display:\s*none/);
