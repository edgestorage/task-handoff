...
test("conversation Timeline composes every turn from the same compact result component", () => {
  ...
  const disclosureTransition = fs.readFileSync(new URL("../src/components/ai-session/disclosureTransition.ts", import.meta.url), "utf8");
  const result = fs.readFileSync(new URL("../src/components/ai-session/AiSessionResult.vue", import.meta.url), "utf8");
  ...
  assert.match(timeline, /watch\(\(\) => props\.turnTimelines,[\s\S]*scheduleStickyUserMessageUpdate/);
  assert.doesNotMatch(timeline, /shouldDeferTurnTimelineLoad/);
  assert.match(timeline, /\.ai-session-timeline-turn \{[\s\S]*gap: 24px;/);
  assert.match(timeline, /@click\.capture="handleDisclosureClick"/);
  assert.match(timeline, /anchorTo: "end"/);
  assert.match(timeline, /watch\(disclosureAnimationActive, \(active\) => \{[\s\S]*shouldAdjustScrollPositionOnItemSizeChange = active \? \(\) => false : undefined/);
  assert.match(timeline, /function handleDisclosureClick[\s\S]*target\.closest\("button\[aria-expanded\]"\)[\s\S]*}, 240\)/);
  assert.match(timeline, /\.ai-session-timeline\.is-disclosure-animating[\s\S]*overflow-anchor: none !important;/);
  assert.match(disclosureTransition, /closest<HTMLElement>\("\[data-task-handoff-scroll-viewport\]"\)/);
  assert.match(disclosureTransition, /viewport\.style\.overflowAnchor = "none"/);
  assert.match(disclosureTransition, /viewport\.style\.overflowAnchor = lock\.previous/);
  assert.match(disclosureTransition, /export function beginDisclosureTransition[\s\S]*pendingScrollAnchorLocks\.set/);
  assert.match(disclosureTransition, /function adoptScrollAnchorLock[\s\S]*transitionViewports\.set/);
  ...
  assert.match(timeline, /anchorTo: "end"/);
  assert.match(timeline, /scrollToFn:[\s\S]*options\.adjustments[\s\S]*disclosureAnimationActive\.value[\s\S]*closest\("\.is-user-layout-changing"\)[\s\S]*return/);
  ...
  assert.match(timeline, /<AiSessionResult[\s\S]*<template #turn-footer>[\s\S]*<AiSessionTurnActions[\s\S]*<\/AiSessionResult>/);
  ...
  assert.match(timeline, /:content="turns\[virtualTurn\.index\]\.latestResponse\?\.text \|\| ''"/);
  ...
  assert.match(timeline, /detachViewportListeners\(scrollElement\.value\)/);
  ...
  assert.match(group, /class="ai-session-activity-group-summary" :aria-expanded="groupOpen" @click="toggleGroup"/);
  assert.match(group, /<Transition[\s\S]*name="activity-disclosure"[\s\S]*@before-enter="prepareDisclosureEnter"[\s\S]*@after-leave="finishDisclosureLeave"/);
  assert.match(group, /<button v-if="hasDetails\(activity\)" type="button" class="ai-session-activity-item-head" :aria-expanded="activityOpen\(activity\)"/);
  ...
  assert.match(history, /\.ai-session-turn-history-disclosure \{[\s\S]*padding-left: 7px; \}/);
  assert.match(history, /\.ai-session-turn-history-content \{[\s\S]*gap: 12px;[\s\S]*padding: 12px 0 0 12px;[\s\S]*border-left: 1px solid var\(--line-subtle\);/);
  assert.doesNotMatch(history, /\.ai-session-turn-history-content \{[\s\S]*margin: 12px 0 0 7px;/);
