...
test("topbar navigation uses fill states without nested control borders", async () => {
  ...
  assert.match(appStyles, /:root:not\(\.dark\):not\(\[data-theme="dark"\]\),[\s\S]*?--workbench-view-switcher-bg: #f2f2f7;[\s\S]*?--workbench-view-selection-bg: #ffffff;/);
  ...
  assert.match(styles, /\.workbench-view-option\.active \{[^}]*background: transparent;[^}]*color: hsl\(var\(--accent-foreground\)\);/);
  assert.match(styles, /\.control-plane-settings-trigger\.inline-flex \{[^}]*height: 32px;[^}]*margin-left: 12px;[^}]*border: 0;[^}]*background: transparent;[^}]*box-shadow: none;[^}]*padding-inline: 9px;/);
  assert.match(styles, /\.control-plane-settings-trigger\.inline-flex::before \{[^}]*left: -8px;[^}]*width: 1px;[^}]*height: 16px;/);
  assert.match(styles, /\.control-plane-shell\[data-header-density="compact"\] \.workbench-view-switcher,[\s\S]*?\.control-plane-shell\[data-header-density="compact"\] \.control-plane-settings-trigger\.inline-flex \{\s*height: 30px;/);
});
