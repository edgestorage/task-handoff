...
test("proxy invite is target-bound and returns plaintext only in its creation result", () => {
  ...
  assert.deepEqual(persisted.invites, []);
  assert.equal(JSON.stringify(persisted).includes(created.token), false);
  assert.equal(JSON.stringify(persisted).includes(crypto.createHash("sha256").update(created.token).digest("base64url")), false);
...
test("claim stores only the A-generated credential hash and exposes only public records", () => {
  ...
  assert.deepEqual(service.listInvites(), []);
...
test("a rejected first claim does not consume the one-time invite", () => {
  const { service } = fixture();
  const invite = service.createInvite({ targetNodeId: "node_a" }, "user_admin");

  assert.throws(
    () => service.claimInvite(claim(invite, { targetNodeId: "node_b" })),
    assertCode(ControlPlaneProxyErrorCode.TargetMismatch),
  );
  assert.equal(service.listInvites().some((candidate) => candidate.id === invite.invite.id), true);
...
test("claim is exactly idempotent and rejects target or identity substitution without partial writes", async () => {
  ...
  assert.throws(
    ...
    assertCode(ControlPlaneProxyErrorCode.InviteInvalid),
  );
  const secondInvite = service.createInvite({ targetNodeId: "node_a" }, "user_admin");
  assert.equal(
    service.claimInvite({ ...input, inviteToken: secondInvite.token }).binding.id,
    first.binding.id,
  );
  assert.equal(service.listInvites().some((candidate) => candidate.id === secondInvite.invite.id), true);
  assert.equal(store.listBindings().length, 1);
});

test("expired and revoked invites are removed like other one-time join tokens", () => {
  ...
  assert.throws(() => service.claimInvite(claim(expired)), assertCode(ControlPlaneProxyErrorCode.InviteInvalid));
...
test("proxy invites use the join-token ten minute ceiling and do not survive restart", () => {
  let now = new Date("2026-08-01T00:00:00.000Z");
  const { authorityFile, service, targets } = fixture({ now: () => now });
  const invite = service.createInvite({ targetNodeId: "node_a", expiresInSeconds: 600 }, "user_admin");
  assert.equal(invite.invite.expiresAt, "2026-08-01T00:10:00.000Z");

  const restarted = new ControlPlaneProxyService(
    new ControlPlaneProxyStore(authorityFile),
    { get: (nodeId) => targets.get(nodeId) },
    { proxyOrigin: "https://proxy.example.test", now: () => now },
  );
  restarted.init();
  assert.deepEqual(restarted.listInvites(), []);
  assert.throws(() => restarted.claimInvite(claim(invite)), assertCode(ControlPlaneProxyErrorCode.InviteInvalid));
});

test("claim consumes the in-memory invite and commits only the binding", () => {
  ...
  assert.deepEqual(authority.invites, []);
  assert.deepEqual(service.listInvites(), []);
...
test("binding authentication is isolated by binding, source, key and credential and respects revocation", () => {
  ...
  assert.throws(() => service.revokeBinding(bindingA.id), assertCode(ControlPlaneProxyErrorCode.BindingUnknown));
  assert.throws(() => service.authenticateBinding(bindingA.id, {
    sourceControlPlaneId: "control_plane_a",
    bindingKeyId: "binding_key_a",
    credential,
  }), assertCode(ControlPlaneProxyErrorCode.BindingUnknown));
  assert.equal(service.listBindings().some((binding) => binding.id === bindingA.id), false);
});
...
test("stored proxy records discard historical invites, revoked bindings, and future fields", () => {
  const { authorityFile, service } = fixture();
  const binding = service.claimInvite(claim(service.createInvite({ targetNodeId: "node_a" }, "user_admin"))).binding;
  const persisted = JSON.parse(fs.readFileSync(authorityFile, "utf8"));
  persisted.futureAuthorityField = true;
  persisted.invites.push({ id: "old_invite", tokenHash: "sensitive-hash", status: "expired" });
  persisted.bindings[0].futureField = true;
  persisted.bindings.push({ ...persisted.bindings[0], id: "old_revoked", status: "revoked", revokedAt: persisted.bindings[0].updatedAt });
  ...
  restarted.init();
  assert.deepEqual(restarted.snapshot().invites, []);
  assert.deepEqual(restarted.listBindings().map((item) => item.id), [binding.id]);
  assert.equal(fs.readFileSync(authorityFile, "utf8").includes("sensitive-hash"), false);
