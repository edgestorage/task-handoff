          ...
          {storySessions.map((entry) => {
            ...
            <Pressable accessibilityRole="button" onPress={() => onOpenSession(entry.instanceId, entry.session.id)} style={({ pressed }) => [styles.childRow, pressed && { backgroundColor: colors.surfaceMuted }]}>
              {entry.session.status === 'running'
                ...
                : <View style={styles.sessionIcon}><SystemIcon android="chat_bubble_outline" color={colors.textMuted} ios="bubble.left" size={18} /><View style={styles.sessionIconStatus}><SessionStatusIndicator group={statusGroup} label={mobileAiSessionStatusLabel(entry.session, t)} size={16} /></View></View>}
