...
export function StoryDocumentPreview({ storyId, nodeId, storyPath, title }: { storyId?: string; nodeId?: string; storyPath?: string; title?: string }) {
  ...
  const [preview, setPreview] = useState<StoryContentPreview>();
  const [error, setError] = useState<string>();
  const refresh = useCallback(async (signal: AbortSignal) => {
    if (!runtime.api || !storyId || !nodeId || !storyPath) return;
    try {
      const next = await runtime.api.stories.preview(storyId, nodeId, storyPath, signal);
  ...
  }, [nodeId, runtime.api, storyId, storyPath]);
  useStoryEvents(refresh, storyId);
  const extension = storyPath?.split('.').pop()?.toLocaleLowerCase() || '';
  const content = useMemo(() => {
    if (!preview) return '';
    if (extension !== 'json') return preview.content;
    try {
