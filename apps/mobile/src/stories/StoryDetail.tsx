...
function Section({ children, headerAction, title }: { children: ReactNode; headerAction?: ReactNode; title: string }) {
  const { colors } = useMobileTheme();
  return <View style={styles.section}><View style={styles.sectionHeader}><Text style={[styles.sectionTitle, { color: colors.text }]}>{title}</Text>{headerAction}</View>{children}</View>;
}
...
const styles = StyleSheet.create({
  ...
  section: { gap: 10 }, sectionHeader: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', minHeight: 28 }, sectionTitle: { fontSize: 18, fontWeight: '600' },
