...
export function StoryDetail({ storyId, nodeId, onOpenSession }: { storyId?: string; nodeId?: string; onOpenSession(instanceId: string, sessionId: string): void }) {
  ...
  const hasUnassignedSessions = useMemo(() => (sessions?.instances ?? []).some((entry) => storyInstanceIds.has(entry.instanceId) && entry.aiSessions.sessions.some((session) => !session.storyId)), [sessions, storyInstanceIds]);
