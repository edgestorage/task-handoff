...
export class MobileAiSessionActionCoordinator {
  ...
  private authoritativeFingerprint(instanceId: string, sessionId: string, queueId?: string) {
    ...
    return JSON.stringify({ title: session.title, status: session.status, phase: session.phase, updatedAt: session.updatedAt, actions: session.actions, queue: session.queue, modelSelection: session.modelSelection });
  }
